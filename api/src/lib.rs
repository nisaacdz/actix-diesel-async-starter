...
use infra::services::{auth::AuthService, frogsms::FrogSmsProvider};
...
pub async fn run(settings: Settings) -> std::io::Result<()> {
    ...
    let sms_provider = web::Data::new(
        FrogSmsProvider::new(&settings.frogsms).expect("Failed to initialize sms provider"),
    );
    ...
    let redis_service = web::Data::new(
        infra::services::redis::RedisService::new(&settings.redis)
            .await
            .expect("Failed to initialize redis service"),
    );

    let bg_worker = actix_web::web::Data::new(infra::services::bg_worker::BgWorker::new());

    let cron_worker = infra::services::cron_worker::CronWorker::new();

    let server = {
        let bg_worker = bg_worker.clone();
        HttpServer::new(move || {
            let pool = pool.clone();
            let auth_service = auth_service.clone();
            let app_settings = app_settings.clone();
            let sms_provider = sms_provider.clone();
            let redis_service = redis_service.clone();
            let bg_worker = bg_worker.clone();

            let mut cors = Cors::default();
            for origin in app_settings.clients.allowed_origins() {
            ...
            App::new()
                ...
                .app_data(sms_provider)
                .app_data(redis_service)
                .app_data(bg_worker)
                .app_data(
    ...
    tracing::info!("Shutting down active HTTP background jobs...");
    bg_worker.join_all().await;

    tracing::info!("Shutting down scheduled background crons...");
    cron_worker.stop();
    cron_worker.join_all().await;
...
async fn healthcheck(
    pool: web::Data<infra::db::DbPool>,
    redis: web::Data<infra::services::redis::RedisService>,
) -> actix_web::HttpResponse {
    let result = app::healthcheck::check_health(&pool, &redis).await;
